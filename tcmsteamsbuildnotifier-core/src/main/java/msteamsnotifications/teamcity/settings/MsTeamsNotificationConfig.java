...
import msteamsnotifications.teamcity.settings.converter.MsTeamsNotificationBuildStateConverter;

import java.util.*;

import static msteamsnotifications.teamcity.BuildStateEnum.*;
...
public class MsTeamsNotificationConfig {
	...
	private static final String SLACK_USER_ENABLED_MESSAGE = "mention-msteams-user-enabled";
	...
	public MsTeamsNotificationConfig(Element e) {
		...
		if (e.getAttribute("statemask") != null){
			this.setBuildStates(MsTeamsNotificationBuildStateConverter.convert(Integer.parseInt(e.getAttributeValue("statemask"))));
		}
		...
		if (e.getAttribute(SLACK_USER_ENABLED_MESSAGE) != null){
			this.setMentionMsTeamsUserEnabled(Boolean.parseBoolean(e.getAttributeValue("mention-msteams-user-enabled")));
		}
    ...
    /**
     * MsTeamsNotificationsConfig constructor. Unchecked version. Use with caution!!
     * This constructor does not check if the payloadFormat is valid.
     * It will still allow you to add the format, but the msteamsnotifications might not
     * fire at runtime if the payloadFormat configured is not available.
     *
     * @param enabled
     * @param states
     */
    public MsTeamsNotificationConfig(String token,
								   Boolean enabled,
								   BuildState states,
								   ...
								   boolean mentionMsTeamsUserEnabled) {
        this.content = new MsTeamsNotificationContentConfig();
        int Min = 1000000, Max = 1000000000;
        Integer Rand = Min + (int) (Math.random() * ((Max - Min) + 1));
        this.uniqueKey = Rand.toString();
		...
		this.setMentionMsTeamsUserEnabled(mentionMsTeamsUserEnabled);
	...
	public Element getAsElement(){
		Element el = new Element("msteamsNotification");
		...
		el.setAttribute(SLACK_USER_ENABLED_MESSAGE, String.valueOf(this.getMentionMsTeamsUserEnabled()));
...
//			}
	...
	public String getMsTeamsNotificationEnabledAsChecked() {
		if (this.enabled){
			return "checked ";
		}
		return ""; 
	...
	public boolean getMentionMsTeamsUserEnabled() {
		return mentionMsTeamsUserEnabled;
	}
    ...
    public void setContent(MsTeamsNotificationContentConfig content) {
